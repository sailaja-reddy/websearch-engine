...
public abstract class Parser
{
	 ...
	 */
	public static String getTagType(Tokenizer token, boolean lowerCaseFlag)
	{
		if (token.getType() == Tokenizer.CONTENT)
			return (null);
	...
	private boolean handle_content(BufferedReader in) throws IOException
	{
		...
		Tokenizer token = new Tokenizer(Tokenizer.CONTENT, guts.toString());
	...
	private void handle_tag(int type, BufferedReader in) throws IOException
	{
		...
		Tokenizer token = new Tokenizer(type, guts.toString());
		if (type == Tokenizer.TAG)
			processTag(token);
		else
			processEndTag(token);
	}
	...
	public void parse(Reader r) throws IOException
	{
		...
		while (true)
		{
			...
			else if (buf[0] == '/')
			{
				in.reset();
				read(in, buf, 1);
				handle_tag(Tokenizer.ENDTAG, in);
			}
			else
			{
				in.reset();
				handle_tag(Tokenizer.TAG, in);
			}
	...
	public abstract void processContent(Tokenizer token) throws IOException;

	/** processEndTag - process an end tag */
	public abstract void processEndTag(Tokenizer token) throws IOException;

	/** processTag - process a tag */
	public abstract void processTag(Tokenizer token) throws IOException;
